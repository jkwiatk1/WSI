...
def expetiment_for_CompleteGraph(num_of_experiment,num_generations):
    ...
    for i in range(num_of_experiment):
        GeneticAlgorithmInit_Complete = GeneticAlgorithm(edges=complete_graph_1_edges, nodes=complete_graph_1_nodes,
                                                         num_generations=num_generations, mutation_probability=0.01, population_size=50,
                                                         tournament_size=2, selection_size=80, mutation_rate=0.01)
        best = GeneticAlgorithmInit_Complete.genetic_algorithm()
        result_cover = best[1]/number_of_all_edges_Complete
        result_for_CompleteGraph.append([best[1],number_of_all_edges_Complete,result_cover])
    return result_for_CompleteGraph


print(expetiment_for_CompleteGraph(5,500))
